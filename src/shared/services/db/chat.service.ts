...
class ChatService {
  public async addMessageToDB(data: IMessageData): Promise<void> {
    ...
    // const conversation: IConversationDocument[] = await ConversationModel.find({ _id: new mongoose.Types.ObjectId(data?.conversationId) }).exec();
  ...
  public async updateMessageReaction(messageId: ObjectId, senderName: string, reaction: string, type: 'add'|'remove'): Promise<void> {
    if (type === 'add') {
      await MessageModel.updateOne({ _id: messageId }, { $push: { reaction: { senderName, type: reaction } } }).exec();   // since its an array we'll use $push instead of $set
    } else {
      await MessageModel.updateOne({ _id: messageId }, { $pull: { reaction: { senderName } } }).exec();   // $pull will remove the reactions object
    }
  }
