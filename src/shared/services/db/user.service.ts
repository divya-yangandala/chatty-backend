...
class UserService {
  ...
  // public async updatePassword(userId: string, hashedPassword: string): Promise<void> {
    // await AuthModel.updateOne({ _id: new mongoose.Types.ObjectId(userId) }, { $set: { password: hashedPassword } }).exec();
  ...
  public async updateNotificationSettings(userId: string, settings: INotificationSettings): Promise<void> {
    await UserModel.updateOne({ _id: userId }, { notifications: settings }).exec();
  }
  ...
  public async getTotalUsersInDB(): Promise<number> {
    const totalNumber: number = await UserModel.find({}).countDocuments();
    return totalNumber;
  }
