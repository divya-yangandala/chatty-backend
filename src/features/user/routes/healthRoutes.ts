...
class HealthRoutes {
    ...
    public instance(): Router {
      this.router.get('/instance', async (req: Request, res: Response) => {
        ...
        res.status(HTTP_STATUS.OK).send(`Server is running on EC2 instance with id ${response.data} and process id ${process.pid} as ${moment().format('LL')}`);
      })
      return this.router;
    }
    ...
    public fiboRoutes(): Router {
      this.router.get('/fibo/:num', async (req: Request, res: Response) => {
        ...
        //   ${response.data}
        //   and process id is ${process.pid} on ${moment().format('LL')}`
        // )
        res.status(HTTP_STATUS.OK).send(
          `Fibonacci series of ${num} is ${result} and it took ${end - start}ms with EC2 instance of
          and process id is ${process.pid} on ${moment().format('LL')}`
