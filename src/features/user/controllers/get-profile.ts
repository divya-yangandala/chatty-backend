...
export class Get {
  public async all(req: Request, res: Response): Promise<void> {
    ...
    const newSkip: number = skip === 0 ? 0 :  skip + 1;
    ...
    const followers: IFollowerData[] = await Get.prototype.followers(`${req.currentUser!.userId}`) as IFollowerData[];
  ...
  // looged in user wants to see the profile of another user
  public async profileByUserId(req: Request, res: Response): Promise<void> {
    const { userId } = req.params;
    const cachedUser: IUserDocument = await userCache.getUserFromCache(userId) as IUserDocument;
    const searchedUser: IUserDocument = cachedUser ?
      cachedUser : await userService.getUserById(userId);
    res.status(HTTP_STATUS.OK).json({ message: 'Get user profile by id', user: searchedUser })
  }
  ...
  public async getProfileAndPosts(req: Request, res: Response): Promise<void> {
    ...
    const user: IUserDocument = cachedUser ?
      cachedUser : await userService.getUserById(userId);

    const userPosts: IPostDocument[] = cachedUserPosts.length ?
      cachedUserPosts : await postService.getPosts({ username: userName }, 0, 100, { createdAt: -1 })

    res.status(HTTP_STATUS.OK).json({ message: 'Get user profile and posts', user: user, posts: userPosts})
  }
  ...
  private async usersCount(type: string): Promise<number> {
    const totalUsers = type === 'redis' ?
      await userCache.getTotalUsersInCache() : await userService.getTotalUsersInDB();
    return totalUsers;
  }
  ...
  private async followers(userId: string): Promise<IFollowerData[]> {
    ...
    const result = cachedFollowers.length ?
      cachedFollowers : await followerService.getFollowerData(new mongoose.Types.ObjectId(userId));
    return result;
  }
}
