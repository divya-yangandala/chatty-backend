...
describe('Add', () => {
  ...
  describe('follower', () => {
    it('should call updateFollowersCountInCache', async () => {
      ...
      expect(FollowersCache.prototype.updateFollowersCountInCache).toHaveBeenCalledWith(`${existingUser._id}`, 'followingCount', 1);
    ...
    it('should call saveFollowerToCache', async () => {
      ...
      expect(FollowersCache.prototype.saveFollowerToCache).toHaveBeenCalledWith('followers:6064861bc25eaa5a5d2f9bf4', `${existingUser._id}`);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        message: 'Following user now'
      });
